 ...
 * 
 * LED on pin 2 turns ON when object is in range (close)
 * 
 ...
 * 
 * LED BEHAVIOR:
 * - LED OFF when distance > 40 cm (far)
 * - LED stays OFF until distance < 20 cm
 * - LED turns ON when distance < 20 cm (close)
 * - LED stays ON until distance > 40 cm
 ...
 * - LED Anode (+, longer leg) → Arduino Pin 2
 * - LED Cathode (-, shorter leg) → 220Ω Resistor → Arduino GND
 * 
 * LED CIRCUIT:
 * Pin 2 ---[LED]---[220Ω]--- GND
 *          ↑
 *       Anode (+)
 * 
 * NOTE: Always use a current-limiting resistor (220Ω-1kΩ) with LEDs!
 * 
 * SERIAL PLOTTER VIEW:
